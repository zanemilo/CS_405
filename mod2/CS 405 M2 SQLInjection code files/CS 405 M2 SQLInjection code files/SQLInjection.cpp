...
bool looks_suspicious(const std::string& sql) {
    // catches: --, ;, /* */, OR 1=1, UNION, SELECT *
    static const std::regex sqli(
        "(--|;|/\\*|\\*/|\\b(OR|AND)\\b\\s+\\d+\\s*=\\s*\\d+|\\bUNION\\b|\\bSELECT\\b\\s+\\*)",
        std::regex_constants::icase
    );
    return std::regex_search(sql, sqli);
}
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // Below errors out due to the way the queries are set up, without params.
  // Index 1 refers to first param which is not there.
  //sqlite3_stmt* stmt = nullptr;  // Initialize stmt obj

  //// Prepare statement && Convert sql str to char via .c_str()
  //if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr)) {
  //    std::cerr << "Prepared Statement Failed: " << sqlite3_errmsg(db) << "\n";
  //}

  //if (sqlite3_bind_text(stmt, 1, sql.c_str(), -1, SQLITE_TRANSIENT) != SQLITE_OK) {
  //    std::cerr << "Failed to bind query to statment :" << sqlite3_errmsg(db) << "\n";
  //    sqlite3_finalize(stmt);
  //    return false;
  //}

  if (looks_suspicious(sql)) {
      std::cout << "!! SQL Injection Detected !!" << "\n";
      return false;
  }
